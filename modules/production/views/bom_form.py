...
from PyQt6.QtWidgets import (
    ...
    QHeaderView, QAbstractItemView, QTabWidget, QDialog, QGridLayout
)
...
class MaterialSelectDialog(QDialog):
    ...
    def _filter_items(self, text: str):
        for row in range(self.table.rowCount()):
            match = any(text.lower() in (self.table.item(row, col).text().lower() if self.table.item(row, col) else "") for col in range(2))
...
class BOMFormPage(QWidget):
    ...
    def _setup_materials_table(self):
        columns = [("Sıra", 50), ("Malzeme Kodu", 120), ("Malzeme Adı", 200), ("Miktar", 100),
                   ("Birim", 80), ("Fire %", 80), ("Net Miktar", 100), ("Birim Maliyet", 110), ("Satır Maliyeti", 110)]
    ...
    def _create_cost_tab(self) -> QWidget:
        ...
        cost_frame = QFrame()
        cost_frame.setStyleSheet("QFrame { background-color: rgba(30, 41, 59, 0.3); border: 1px solid #334155; border-radius: 12px; }")
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if item:
                self.lines.append({
                    ...
                    "quantity": Decimal("1"), "unit_code": item["unit_code"], "unit_id": item.get("unit_id"),
                    "scrap_rate": Decimal("0"), "unit_cost": Decimal(str(item["purchase_price"])),
                })
    ...
    def _on_save(self):
        ...
        data = {
            "code": code, "name": name, "description": self.description_input.toPlainText().strip(),
            "item_id": product_id, "base_quantity": Decimal(str(self.base_qty_input.value())),
            "unit_id": self.unit_combo.currentData(), "version": self.version_input.value(),
            "revision": self.revision_input.text().strip() or "A", "status": self.status_combo.currentData(),
            "labor_cost": Decimal(str(self.labor_cost_input.value())),
            "overhead_cost": Decimal(str(self.overhead_cost_input.value())),
            "lines": [{"item_id": l["item_id"], "quantity": l["quantity"], "unit_id": l.get("unit_id"),
                       "scrap_rate": l.get("scrap_rate", Decimal(0)), "unit_cost": l.get("unit_cost", Decimal(0)),
                       "line_cost": l.get("line_cost", Decimal(0))} for l in self.lines]
        }
