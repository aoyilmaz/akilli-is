...
class PurchaseOrderListPage(QWidget):
    ...
    def setup_ui(self):
        ...
        self.total_amount_card = self._create_stat_card(
            "💰", "Toplam Tutar", "₺0", "#8b5cf6"
        )
    ...
    def _display_data(self, orders: list):
        ...
        for row, order in enumerate(orders):
            ...
            # Tedarikçi
            self.table.setItem(
                row, 2, QTableWidgetItem(order.get("supplier_name", "") or "-")
            )
            ...
            # Kalem
            self.table.setItem(
                row, 4, QTableWidgetItem(str(order.get("total_items", 0)))
            )
            ...
            if order_status == "draft":
                ...
                send_btn.clicked.connect(
                    lambda checked, id=order_id: self.send_clicked.emit(id)
                )
                ...
                receive_btn.setIconSize(QSize(16, 16))
                receive_btn.setFixedSize(32, 28)
                receive_btn.setToolTip("Mal Kabul Oluştur")
                receive_btn.clicked.connect(
                    lambda checked, id=order_id: self.create_receipt_clicked.emit(id)
                )
