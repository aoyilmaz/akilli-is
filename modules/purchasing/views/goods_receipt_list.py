...
class GoodsReceiptListPage(QWidget):
    ...
    def setup_ui(self):
        ...
        from_order_btn.setFixedHeight(BTN_HEIGHT_NORMAL)
        from_order_btn.setStyleSheet(get_button_style("secondary"))
        from_order_btn.clicked.connect(self.add_from_order_clicked.emit)
        header_layout.addWidget(from_order_btn)

        # Yeni (manuel)
        add_btn = QPushButton(f"{ICONS['add']} Manuel Giriş")
    ...
    def _display_data(self, receipts: list):
        ...
        for row, rec in enumerate(receipts):
            ...
            # Tedarikçi
            self.table.setItem(
                row, 2, QTableWidgetItem(rec.get("supplier_name", "") or "-")
            )
            ...
            # Depo
            self.table.setItem(
                row, 4, QTableWidgetItem(rec.get("warehouse_name", "") or "-")
            )
            ...
            view_btn.clicked.connect(
                lambda checked, id=rec_id: self.view_clicked.emit(id)
            )
            ...
            if rec_status == "draft":
                ...
                edit_btn.clicked.connect(
                    lambda checked, id=rec_id: self.edit_clicked.emit(id)
                )
                ...
                complete_btn.clicked.connect(
                    lambda checked, id=rec_id: self.complete_clicked.emit(id)
                )
                ...
                del_btn.clicked.connect(
                    lambda checked, id=rec_id: self._confirm_delete(id)
                )
