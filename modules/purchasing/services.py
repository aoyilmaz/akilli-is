...
from database.models.purchasing import (
    ...
    PurchaseRequestStatus, PurchaseOrderStatus, GoodsReceiptStatus, Currency
)
...
class PurchaseRequestService:
    ...
    def get_by_id(self, request_id: int) -> Optional[PurchaseRequest]:
        """ID ile talep getir"""
        return self.session.query(PurchaseRequest).options(
    ...
    def update(self, request_id: int, items_data: List[Dict] = None, **data) -> Optional[PurchaseRequest]:
        """Talep güncelle"""
    ...
    def generate_request_no(self) -> str:
        ...
        last = self.session.query(PurchaseRequest).filter(
...
class PurchaseOrderService:
    ...
    def get_all(self, status: PurchaseOrderStatus = None, 
                supplier_id: int = None) -> List[PurchaseOrder]:
    ...
    def get_by_id(self, order_id: int) -> Optional[PurchaseOrder]:
        ...
        return self.session.query(PurchaseOrder).options(
            joinedload(PurchaseOrder.supplier),
    ...
    def create_from_request(self, request_id: int, supplier_id: int, 
                           items_data: List[Dict], **data) -> PurchaseOrder:
    ...
    def update(self, order_id: int, items_data: List[Dict] = None, **data) -> Optional[PurchaseOrder]:
        """Sipariş güncelle"""
    ...
    def cancel(self, order_id: int) -> Optional[PurchaseOrder]:
        """Sipariş iptal"""
        order = self.get_by_id(order_id)
        if order and order.status in [PurchaseOrderStatus.DRAFT, PurchaseOrderStatus.SENT]:
    ...
    def close(self, order_id: int) -> Optional[PurchaseOrder]:
        """Sipariş kapat"""
        order = self.get_by_id(order_id)
        if order and order.status in [PurchaseOrderStatus.RECEIVED, PurchaseOrderStatus.PARTIAL]:
    ...
    def generate_order_no(self) -> str:
        ...
        last = self.session.query(PurchaseOrder).filter(
...
class GoodsReceiptService:
    ...
    def get_by_id(self, receipt_id: int) -> Optional[GoodsReceipt]:
        ...
        return self.session.query(GoodsReceipt).options(
            joinedload(GoodsReceipt.supplier),
            joinedload(GoodsReceipt.warehouse),
        ...
        ).filter(GoodsReceipt.id == receipt_id).first()
    ...
    def create_from_order(self, order_id: int, warehouse_id: int,
                         items_data: List[Dict], **data) -> GoodsReceipt:
        ...
        if order.status not in [PurchaseOrderStatus.CONFIRMED, 
                                PurchaseOrderStatus.PARTIAL,
                                PurchaseOrderStatus.SENT]:
        ...
        for grn_item in receipt.items:
            if grn_item.po_item_id:
                ...
                if po_item:
                    po_item.received_quantity = Decimal(str(po_item.received_quantity or 0)) + \
        ...
        try:
            ...
            for item in receipt.items:
                if item.accepted_quantity and item.accepted_quantity > 0:
                    movement_service.goods_receipt(
                        ...
                        notes=f"Mal Kabul: {receipt.receipt_no}"
                    )
    ...
    def generate_receipt_no(self) -> str:
        ...
        last = self.session.query(GoodsReceipt).filter(
            GoodsReceipt.receipt_no.like(f"{prefix}%")
