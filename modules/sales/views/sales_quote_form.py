...
class SalesQuoteFormPage(QWidget):
    """Satış teklif formu"""

    saved = pyqtSignal(dict)
    cancelled = pyqtSignal()
    send_to_customer = pyqtSignal(int)
    ...
    def __init__(
        ...
        self.is_edit_mode = quote_data is not None
        self.items = items or []
    ...
    def setup_ui(self):
        ...
        title_text = "Teklif Düzenle" if self.is_edit_mode else "Yeni Satış Teklifi"
        ...
        # Müşteriye Gönder butonu (sadece düzenlemede ve taslak ise)
        if self.is_edit_mode and self.quote_data.get("status") == "draft":
            send_btn = QPushButton("📤 Müşteriye Gönder")
            ...
            """)
            send_btn.clicked.connect(self._on_send_to_customer)
        ...
        # Teklif No
        general_layout.addWidget(self._create_label("Teklif No"), row, 0)
        self.quote_no_input = QLineEdit()
        self.quote_no_input.setPlaceholderText("Otomatik oluşturulacak")
        self.quote_no_input.setReadOnly(True)
        self._style_input(self.quote_no_input)
        general_layout.addWidget(self.quote_no_input, row, 1)
        row += 1

        # Teklif Tarihi
        general_layout.addWidget(self._create_label("Teklif Tarihi *"), row, 0)
        ...
        self._style_date(self.quote_date_input)
        general_layout.addWidget(self.quote_date_input, row, 1)
        row += 1
        ...
        # Geçerlilik Tarihi
        general_layout.addWidget(self._create_label("Geçerlilik Tarihi"), row, 0)
        ...
        self._style_date(self.valid_until_input)
        general_layout.addWidget(self.valid_until_input, row, 1)
        row += 1
        ...
        # === TEKLİF KALEMLERİ ===
        items_frame = self._create_section("📦 Teklif Kalemleri")
        ...
        self.discount_label.setStyleSheet("color: #ef4444; font-weight: bold;")
    ...
    def _create_section(self, title: str) -> QFrame:
        ...
        title_label.setStyleSheet(
            "font-size: 16px; font-weight: bold; color: #f8fafc; background: transparent; border: none;"
        )
    ...
    def _select_customer(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted and dialog.selected_customer:
            ...
            self.customer_input.setText(
                f"{dialog.selected_customer['code']} - {dialog.selected_customer['name']}"
            )
    ...
    def _update_totals(self):
        ...
        for row in range(self.items_table.rowCount()):
            ...
            if qty_widget and price_widget:
                ...
                discount_rate = Decimal(str(discount_widget.value())) if discount_widget else Decimal("0")
        ...
        # KDV hesapla (ara toplam - iskonto üzerinden)
    ...
    def load_data(self):
        ...
        self.quote_no_input.setText(self.quote_data.get("quote_no", ""))

        quote_date = self.quote_data.get("quote_date")
        if quote_date:
            if isinstance(quote_date, date):
                self.quote_date_input.setDate(
                    QDate(quote_date.year, quote_date.month, quote_date.day)
                )

        # Müşteri
        customer_id = self.quote_data.get("customer_id")
        customer_name = self.quote_data.get("customer_name", "")
        customer_code = self.quote_data.get("customer_code", "")
        if customer_id:
            self.selected_customer = {"id": customer_id, "name": customer_name, "code": customer_code}
        ...
        if valid_until:
            if isinstance(valid_until, date):
                self.valid_until_input.setDate(
                    QDate(valid_until.year, valid_until.month, valid_until.day)
                )

        currency = self.quote_data.get("currency")
        if currency:
        ...
        self.notes_input.setPlainText(self.quote_data.get("notes", "") or "")

        # Kalemleri yükle
        items_data = self.quote_data.get("items", [])
    ...
    def _on_save(self):
        ...
        qdate = self.quote_date_input.date()
        valid_qdate = self.valid_until_input.date()

        # Currency enum değerini al (TRY, USD, EUR)
        currency_code = self.currency_input.currentText().split(" - ")[0]

        data = {
            "quote_date": date(qdate.year(), qdate.month(), qdate.day()),
            "customer_id": self.selected_customer.get("id"),
            "valid_until": date(valid_qdate.year(), valid_qdate.month(), valid_qdate.day()),
            "currency": currency_code,
            "notes": self.notes_input.toPlainText().strip() or None,
            "items": items_data,
        }

        if self.is_edit_mode and self.quote_data:
            data["id"] = self.quote_data.get("id")

        self.saved.emit(data)

    def _on_send_to_customer(self):
        """Müşteriye gönder"""
        if self.quote_data:
            reply = QMessageBox.question(
                self,
                "Müşteriye Gönder",
                "Bu teklifi müşteriye göndermek istediğinize emin misiniz?\n\nGönderdikten sonra düzenleme yapılamaz.",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if reply == QMessageBox.StandardButton.Yes:
                self.send_to_customer.emit(self.quote_data.get("id"))
