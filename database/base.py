...
def _create_default_data():
    ...
    try:
        ...
        currencies = [
            ...
            {"code": "USD", "name": "Amerikan Doları", "symbol": "$", "is_default": False},
            {"code": "EUR", "name": "Euro", "symbol": "€", "is_default": False},
            {"code": "GBP", "name": "İngiliz Sterlini", "symbol": "£", "is_default": False},
        ]
        
        for curr_data in currencies:
            exists = session.query(Currency).filter(Currency.code == curr_data["code"]).first()
            if not exists:
        ...
        # Roller
        roles = [
            {"code": "ADMIN", "name": "Sistem Yöneticisi", "description": "Tüm yetkilere sahip"},
            {"code": "MANAGER", "name": "Yönetici", "description": "Yönetim yetkileri"},
            {"code": "ACCOUNTANT", "name": "Muhasebeci", "description": "Finans ve muhasebe yetkileri"},
            {"code": "WAREHOUSE", "name": "Depocu", "description": "Stok yönetimi yetkileri"},
            {"code": "SALES", "name": "Satış Temsilcisi", "description": "Satış yetkileri"},
            {"code": "PURCHASE", "name": "Satın Alma", "description": "Satın alma yetkileri"},
            {"code": "PRODUCTION", "name": "Üretim", "description": "Üretim yetkileri"},
            {"code": "VIEWER", "name": "İzleyici", "description": "Sadece görüntüleme yetkisi"},
        ]
        ...
        for perm_data in permissions:
            exists = session.query(Permission).filter(Permission.code == perm_data["code"]).first()
            if not exists:
